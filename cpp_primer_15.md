...
静态类型是变量声明的类型或表达式产生的类型，在编译期间就已经知道了。动态类型是变量或表达式表示的内存中对象的类型，运行时才知道。不是指针或引用的表达式的动态类型总是和静态类型一样。
...
派生类中的虚函数必须和基类中虚函数参数一样。除了一个例外，派生类虚函数的返回类型也必须和基类一样。这个例外就是返回类型为和继承相关的指针或引用。
